...
public class SelectTimeDialog extends Dialog implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cancelBtn = findViewById(R.id.dialog_time_btn_cancle);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.dialog_time_btn_cancle:
                cancel();
                break;
            case R.id.dialog_time_btn_ensure:
                int year = datePicker.getYear();
                int month = datePicker.getMonth()+1;
                int dayOdMonth = datePicker.getDayOfMonth();
                String monthStr = String.valueOf(month);
                if(month<10){
                    monthStr = "0"+month;
                }
                String dayStr = String.valueOf(dayOdMonth);
                if (dayOdMonth<10){
                    dayStr="0"+dayOdMonth;
                }
                ...
                if (onEnsureListener!=null) {
                    onEnsureListener.onEnsure(timeFormat,year,month,dayOdMonth);
                }
                cancel();
                break;
        }
